            ...
            steps {
                script {
                    ...
                    try {
                        ...
                        bat "MSYS_NO_PATHCONV=1 docker exec ${containerId} sh -c 'set -x; composer install || { echo \"Fehler bei der Composer-Installation!\"; exit 1; }'"

                        // 2. Unit-Tests ausführen
                        bat "MSYS_NO_PATHCONV=1 docker exec ${containerId} sh -c 'set -x; ./vendor/bin/phpunit || { echo \"Fehler bei der Ausführung der Unit-Tests!\"; exit 1; }'"
                    } finally {
